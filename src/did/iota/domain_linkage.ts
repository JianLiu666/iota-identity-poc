...
async function domainLinkage() {
  ...
  const [unpublished, vmFragment1] = await createDocument(storage);
  ...
  let configurationResource: DomainLinkageConfiguration = new DomainLinkageConfiguration([
    credentialJwt,
  ]);
  ...
  const issuerDocument: IotaDocument = await identityClient.resolveDid(
    IotaDID.parse(issuers[0].toString()),
  );
